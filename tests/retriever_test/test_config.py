"""
Unit tests for the retriever configuration module.

This module tests the RetrieverConfig class and all configuration presets
...
class TestRetrieverConfig:
    ...
    def test_config_validation_port(self):
        ...
        # Valid ports
        config1 = RetrieverConfig(port=1)
        config2 = RetrieverConfig(port=65535)
        config3 = RetrieverConfig(port=6333)
        
        assert config1.port == 1
        assert config2.port == 65535
        assert config3.port == 6333
        
        # Invalid ports
        with pytest.raises(ValidationError):
            RetrieverConfig(port=0)
        
        with pytest.raises(ValidationError):
            RetrieverConfig(port=65536)
    
    def test_config_validation_weights(self):
        """Test weight validation."""
        # Valid weights
        config1 = RetrieverConfig(semantic_weight=0.0, keyword_weight=1.0)
        config2 = RetrieverConfig(semantic_weight=1.0, keyword_weight=0.0)
        config3 = RetrieverConfig(semantic_weight=0.5, keyword_weight=0.5)
        
        assert config1.semantic_weight == 0.0
        assert config1.keyword_weight == 1.0
        assert config2.semantic_weight == 1.0
        assert config2.keyword_weight == 0.0
        assert config3.semantic_weight == 0.5
        assert config3.keyword_weight == 0.5
        
        # Invalid weights
        with pytest.raises(ValidationError):
            RetrieverConfig(semantic_weight=-0.1)
        
        with pytest.raises(ValidationError):
            RetrieverConfig(keyword_weight=1.1)
    ...
    def test_config_str_representation(self):
        ...
        assert "strategy=semantic" in config_str
...
class TestConfigurationEdgeCases:
    ...
    def test_config_with_none_values(self):
        ...
        assert config.reranking_model is None
    ...
    def test_config_with_complex_parameters(self):
        """Test configuration with complex parameters."""
        complex_filters = {
            "category": ["AI", "ML"],
        ...
        config = RetrieverConfig(
            default_filters=complex_filters,
            custom_parameters={"param1": "value1", "param2": 42}
        )
        
        assert config.default_filters == complex_filters
        assert config.custom_parameters == {"param1": "value1", "param2": 42}
    
    def test_config_immutability(self):
        """Test that configuration updates create new instances."""
        original_config = RetrieverConfig(max_results=10)
        updated_config = original_config.update(max_results=20)
        
        assert original_config.max_results == 10
        assert updated_config.max_results == 20
